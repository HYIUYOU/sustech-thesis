...
PD分离架构将Prefill与Decode阶段分配至不同GPU实例，消除了两阶段间的直接资源竞争，却并未解决更深层的资源管理矛盾。第一章将这一矛盾归纳为相互关联的\textbf{挑战②}和\textbf{挑战③}：Prefill与Decode阶段在计算和显存上的需求天然互补，但现有系统无法在运行时动态利用这种互补性；与此同时，调度决策与KV Cache的物理位置紧耦合，使缓存感知路由持续向高命中率实例倾斜，在动态异构负载下难以维持全局均衡。两类问题的根源指向同一设计缺陷，即计算资源与KV Cache状态绑定于特定实例，负载均衡与缓存局部性之间由此产生了难以调和的结构性矛盾。

就挑战②而言，Prefill实例以处理长输入序列为主，计算利用率通常高达95\%，但显存利用率仅约35\%；Decode实例的情况恰好相反，计算利用率约35\%，显存利用率却接近90\%。这种天然的计算--显存互补性在现有系统中几乎无法被动态利用：当Prefill实例计算饱和时，其大量闲置显存无法转让给同时显存紧张的Decode实例，资源的互补性停留在存在而非可用的层面。

就挑战③而言，以SGLang\cite{zheng2024sglang}和vLLM\cite{kwon2023efficient}为代表的系统在路由请求时优先考虑KV Cache命中率，将请求倾向于发往缓存命中率高的Prefill实例。这一策略在静态负载下能够降低重复计算，但在动态异构负载下会形成持续性热点：命中率高的实例持续吸引更多请求并积累更多缓存内容，进一步提高命中率，而其他实例却因缓存内容少而长期处于轻载状态，形成正反馈循环，系统整体吞吐量因此受限。

针对上述两类挑战，本章提出\textbf{BanaServe}框架，以解耦资源分配与缓存状态管理为核心设计原则，通过三项互补的技术机制加以实现。针对\textbf{挑战②}，框架引入粗细两种粒度的自适应迁移机制：\textbf{层级权重迁移}（Layer-level Weight Migration）在Transformer层粒度上同步迁移权重与KVCache，承担跨实例的粗粒度计算--显存资源再平衡；\textbf{注意力级KV Cache迁移}（Attention-level KV Cache Migration）沿注意力头维度分割KV状态并迁移至辅助GPU，无需传输模型权重，可在毫秒级内完成细粒度负载调整，适合实时性要求高的动态场景。针对\textbf{挑战③}，框架构建\textbf{全局KV
Cache存储}（Global KV Cache Store），将KV Cache从特定Prefill实例解放出来，使路由器无需考虑缓存位置即可实现纯负载感知调度；配合层间流水线重叠传输技术，将KV Cache预取操作（约0.082\,ms/层）隐藏于前向计算（约4.22\,ms/层）之中，消除全局存储引入的访问延迟瓶颈。
...
\begin{figure}[htbp]
    ...
    \caption{HFT和vLLM在不同请求速率下的GPU资源利用率对比}
    \label{fig:bana_gpu_util}
\end{figure}
现有LLM推理系统在静态分配策略下频繁出现资源利用率不足的问题，其根源在于两个方面：（1）在低请求率（RPS $\leq$ 10）条件下，系统大量计算和显存资源处于闲置状态；（2）固定资源分配与LLM推理动态工作负载特性之间存在根本性的失配——满足存储需求往往制约可用的计算能力，反之亦然。
...
\begin{enumerate}
    \item \textbf{实例1（Instance 1）}：命中率最高，接收大多数查询，计算利用率达100\%，缓存前缀Q1--Q5持续扩大，形成正反馈；
    \item \textbf{实例2（Instance 2）}：接收较少查询，计算利用率仅40\%，存储的前缀Q6--Q7与其他实例存在冗余；
    \item \textbf{实例3（Instance 3）}：计算利用率60\%，主要处理未缓存前缀Q8--Q10，需重复计算，丧失缓存收益。
\end{enumerate}
...
前缀缓存感知路由器的核心瓶颈在于前缀缓存存储在Prefill节点间的不均衡分布，导致路由器必须同时考虑计算负载和缓存位置。为解决这一问题，BanaServe引入全局KV Cache存储（Global KV Cache Store），跨所有Prefill和Decode节点共享KV Cache。
...
\begin{figure}[htbp]
    ...
    \caption{BanaServe全局KV Cache存储的管理与复用机制}
    \label{fig:bana_kvcache_store}
\end{figure}

\item \textbf{层级流水线重叠传输}
...
\begin{figure}[htbp]
    ...
    \caption{BanaServe三阶段层级KV Cache流水线验证}
...
\item \textbf{算法执行分四个阶段：}

\textbf{阶段一：负载测量（第1--3行）}

第1--3行对设备集合$D$中的每个设备$d$调用MeasureUtilization函数，
测量其当前的综合计算和显存利用率$load[d]$。该函数计算式~\eqref{eq:normalized_util}
定义的归一化综合利用率$U_d = C_d/C^{\max}_d + M_d/M^{\max}_d$，将计算
和显存两个维度的利用率线性叠加，得到取值范围$[0,2.0]$的综合负载指标。
$U_d$越接近2.0，表示该设备的计算资源和显存同时接近饱和，是迁移的优先
来源；$U_d$越接近0，表示该设备双维度均空闲，是接收迁移的理想目标。
此阶段的测量结果作为后续所有决策的实时基准，因此需要在每个控制周期开
始时优先执行，确保决策基于最新的系统状态。

\textbf{阶段二：负载分类（第4--5行）}

第4行构建过载设备集合$overload$，将负载超过当前最轻负载设备超过阈值
$\delta$的设备归入过载类别：$overload = \{d \in D \mid load[d] -
\min(load) > \delta\}$。使用相对阈值（与最轻负载设备的差距）而非绝对
阈值，可以避免在系统整体高负载时将所有设备错误归类为过载，从而在高
吞吐量场景下保持算法的稳定性。

第5行类似地构建欠载设备集合$underload$：$underload = \{d \in D \mid
\max(load) - load[d] > \delta\}$，将负载低于最重负载设备超过$\delta$
的设备归入欠载类别。两个集合的构建均采用同一阈值$\delta$，确保仅在
负载差距确实显著时才触发迁移，避免因细微的负载波动导致频繁的无效
迁移。$\delta$的取值需根据实际集群的负载特性离线标定，通常设为$0.3$
至$0.5$之间。

\textbf{阶段三：迁移决策与执行（第6--13行）}

第6行的While循环条件要求同时存在过载设备$d_o \in overload$和欠载设备
$d_u \in underload$，即只有当系统同时存在"负担过重"和"有余量可接收"
的设备时，才进行迁移决策。若仅有过载设备而无欠载设备（例如全局高负载
时），迁移将加剧欠载设备的负担，因此直接退出；反之亦然。

第7行计算当前过载设备$d_o$与欠载设备$d_u$之间的瞬时负载差$\Delta =
load[d_o] - load[d_u]$。$\Delta$越大，说明两设备间的负载失衡越严重，
迁移的潜在收益越高，同时也表明需要迁移更多模块才能恢复均衡。$\Delta$
将在第8行和第10行作为迁移策略选择的依据，同时参与迁移收益-开销比的
计算。

第8--9行处理层级权重迁移的决策与执行。当$\Delta \geq \delta$（负载差
距超过阈值，值得执行代价较高的层级迁移）且$d_o$支持层级迁移
（SupportsLayerMigration返回真，即过载设备有足够的Transformer层可供
迁移）时，调用MigrateLayer($d_o$, $d_u$)执行粗粒度的层级迁移。该函数
将$d_o$上的若干连续Transformer层的权重$W_\ell$和对应的KV Cache
$\mathcal{K}_\ell$一并迁移至$d_u$，同时迁移数据量为$S^{\text{total}}_\ell
= S^w_\ell + S^{kv}_\ell$（式~\eqref{eq:layer_migration_size}），迁移
延迟约为$T_{\text{layer}} \approx S^{\text{total}}_\ell / B_{\text{net}}
+ T_{\text{sync}}$（式~\eqref{eq:layer_migration_latency}）。层级迁移
适合处理严重的负载失衡场景（$\Delta$较大），单次迁移可转移大量计算和
显存资源，快速恢复全局均衡。执行前须验证迁移的收益-开销比
$\text{Benefit}(m)/\text{Cost}(m) \geq \rho$，
确保迁移确实有助于降低全局负载差距。

第10--11行处理注意力级KV Cache迁移的决策与执行。当$\Delta \geq \delta$
但$d_o$不支持层级迁移（例如可供迁移的层数已不足）时，转为尝试细粒度
的注意力级迁移。若SupportsAttentionMigration返回真（过载设备有可分割
的注意力头），调用MigrateKVHeads($d_o$, $d_u$)将$d_o$上部分注意力头
的KV Cache（$K^{(2)}, V^{(2)}$）迁移至$d_u$进行远程并行计算。由于仅
迁移KV状态而无需传输模型权重，迁移数据量远小于层级迁移
（$S_{kv} \ll S^{\text{total}}_\ell$），迁移延迟约为$T_{\text{attn}}
\approx S_{kv}/B_{\text{net}} \ll T_{\text{layer}}$
（式~\eqref{eq:attn_migration_latency}）。注意力级迁移适合处理轻度的
细粒度负载波动，在不引发显著服务中断的前提下实时调整计算分配。

第12行在每次迁移完成后立即更新$load[d_o]$和$load[d_u]$，反映迁移后
的预期负载变化。具体地，$load[d_o]$减少迁移的计算和显存贡献量，
$load[d_u]$相应增加，确保While循环的下一次迭代基于最新的负载状态
做出决策，避免因信息滞后导致的过度迁移。

第13行相应更新模块分配方案$cfg'$，记录新的层到GPU的映射关系和KV
Cache的分布位置，供后续推理计算和调度决策使用。$cfg'$的持续更新确保
BanaServe的所有组件（调度器、迁移引擎、KV Cache存储）始终基于一致的
系统状态运行。

\textbf{阶段四：返回更新方案（第14--15行）}

While循环终止时，系统中已不再存在满足同时过载和欠载条件的设备对，即
当前负载已达到$\delta$阈值定义下的均衡状态。第14--15行返回最终更新的
模块分配方案$cfg'$，供BanaServe的执行引擎（Execution Engine）按照新
的设备映射关系部署后续的推理计算任务。

在收敛性保证方面，算法在每次迭代中必然将某台过载设备的负载降低并将
某台欠载设备的负载提升，且每次迁移后至少有一组过载-欠载设备对的负载
差距缩小至$\delta$以下，因此迭代次数有界，算法在有限步内收敛。为防止
在负载边界附近产生振荡，实际实现采用滞后控制机制，引入不同的上升阈值
$\delta_\uparrow$和下降阈值$\delta_\downarrow$（$\delta_\uparrow >
\delta_\downarrow$），仅当负载差距超过$\delta_\uparrow$时才触发迁移，
仅当差距恢复至$\delta_\downarrow$以下时才停止，有效避免了迁移决策的
频繁切换。


\item \textbf{复杂度与收敛性分析}
...
第1--4行对Prefill实例集合$P$中的每个实例$p_i$执行两项测量。第2行调用
MeasureUtilization测量$p_i$的综合负载$load[p_i]$，计算方式与
式~\eqref{eq:prefill_load}一致：$U_{p_i} = C_{p_i}/C^{\max}_{p_i} +
M_{p_i}/M^{\max}_{p_i}$，将计算和显存两个维度叠加为单一标量指标。
相比单独使用计算利用率或显存利用率，综合指标能够更全面地反映Prefill
实例的实际承载能力：若仅关注计算利用率，可能将高显存压力但计算空闲的
实例误判为轻载；若仅关注显存利用率，则可能忽略已处于计算饱和状态的实
例。

第3行调用GetQueueLength获取$p_i$当前的等待队列长度$q\_len[p_i]$，记录
尚未开始处理的积压请求数。队列长度作为辅助指标，在多个实例综合利用率
相近时用于打破平局：负载相近的实例中，队列更短的实例能够更快开始处理
新请求，应被优先选择。两项指标共同构成调度决策的信息基础，确保分配结
果兼顾当前负载状态和近期处理能力。

与传统前缀缓存感知路由器不同，此阶段完全不采集缓存命中率信息。由于
全局KV Cache存储层已将所有Prefill实例的缓存统一管理，任意实例均可透明
地访问所有已缓存的KV数据（通过层级流水线重叠传输，访问延迟几乎被完全
隐藏），因此路由决策无需考虑缓存局部性，从根本上避免了前缀缓存感知路
由引发的热点正反馈问题。

\textbf{阶段二：实例排序（第5行）}

第5行将$P$中所有Prefill实例按综合负载$load[p_i]$和队列长度$q\_len[p_i]$
升序排列，生成有序的候选实例列表$candidates$。排序键的优先级为：综合
负载$load[p_i]$作为主键（负载越低越优先），队列长度$q\_len[p_i]$作为
次键（同负载下队列越短越优先）。这一排序设计使得后续调度循环在选择目
标实例时无需每次重新比较所有实例，而是直接从列表头部取最轻负载实例，
将调度决策的时间复杂度从$O(|P|)$降至$O(1)$。排序本身的复杂度为
$O(|P| \log |P|)$，在每个调度周期执行一次，适合轻量级实时调度。

注意：此排序基于测量时刻的快照状态执行，在整个调度循环（阶段三）中
排序结果可能因实例负载更新而逐渐偏离实时状态。然而，由于阶段三中每
次分配后立即更新$load[p_{target}]$（第10行），排序结果的偏差仅在批量
分配的后期才会显著影响决策质量。在实际部署中，通过设置合理的调度周期长度（通常为
50--100 ms），可确保每次排序时获取足够新鲜的负载状态。

\textbf{阶段三：逐请求调度循环（第6--13行）}

第6行开始遍历请求队列$Q$中的每个请求$req$，依次为其选择目标Prefill实
例并完成分配。

第7行从已排序的$candidates$列表中按最轻负载策略（least-loaded）选取目
标实例$p_{target}$，即当前综合负载最低的Prefill实例。由于$candidates$
在阶段二已按负载升序排列，此操作直接取列表头部元素，无需额外比较，时
间复杂度为$O(1)$。

第8--10行处理目标实例负载未超过阈值$\delta_L$的标准情形。第8行检查
$load[p_{target}] < \delta_L$，其中$\delta_L$为预设的负载上限阈值，
防止调度器将过多请求集中分配至同一实例而造成局部过载。若目标实例综合
负载在可接受范围内，第9行调用AssignRequest将$req$正式分配给$p_{target}$，
触发该实例开始调度$req$的Prefill计算。第10行随即更新$load[p_{target}]$，
将$req$的预估负载贡献通过EstimateLoad($req$)叠加至当前记录值。
EstimateLoad根据$req$的输入长度（$\text{Input}_i$）和预测输出长度
（$\widehat{\text{Output}}_i$，由资源画像器提供）估算该请求的计算量
和KV Cache显存占用，将其折算为归一化负载增量后加至$load[p_{target}]$。
这一即时更新机制确保后续请求的调度决策能够感知到当前请求带来的负载
增量，避免在同一轮循环中将多个重型请求全部分配至同一实例。

第11--12行处理目标实例已达到负载上限（$load[p_{target}] \geq \delta_L$）
的降级情形。此时所有实例均处于较重的负载状态，理想的最轻负载实例已无
法在阈值内接受新请求。第11行切换策略，从$candidates$中按最短队列策略
（lowest-queue）重新选取目标实例，即选择当前等待队列最短的实例作为分
配目标。队列最短的实例意味着已积压的待处理请求最少，即便其综合利用率
较高，也能相对最快地开始处理新请求，从而最小化$req$的等待延迟。第12行
执行分配操作，将$req$追加至所选实例的等待队列末尾。与标准情形不同，
此处不更新$load$值，因为$req$尚未开始处理，其实际负载贡献无法确定，
贸然更新$load$可能影响后续请求的调度准确性。

第13行在标准情形（第9行分配后）已隐式完成了$load$更新，此处无需额外
操作即可进入下一个请求$req$的调度，实现了$O(1)$的单请求调度复杂度。

\textbf{阶段四：生成调度方案（第14--15行）}

第14行在遍历完$Q$中所有请求并完成分配后，调用GetDispatchMap($P$)汇总
各Prefill实例的最终分配结果，生成调度映射$D^*$。$D^*$以字典形式记录
每个请求与其目标Prefill实例的对应关系，以及目标实例在接收新请求后的
预期负载状态，供BanaServe的编排组件（Orchestration Layer）按此映射关
系将请求实际路由至对应实例的推理队列。

第15行返回$D^*$，完成本调度周期的全部决策。由于BanaServe的调度器以
周期性方式运行（每50--100 ms执行一次），$D^*$仅在当前周期内有效；下
一周期开始时，算法将重新测量所有实例的最新负载状态并重新排序，确保
调度决策持续追踪系统的动态变化。

\item \textbf{调度策略的正确性保证}

与自适应模块迁移算法共同运行时，负载感知调度算法能够维持以下系统级
性质：（1）\textbf{无缓存热点}：由于调度决策完全基于实时负载而非缓存
命中率，任何实例均不会因高缓存命中率而持续吸引过量请求，从根本上打
破了前缀缓存感知路由的正反馈循环；（2）\textbf{负载均衡稳定性}：即时
更新$load[p_{target}]$机制确保同一调度周期内的多次分配不会造成单实例
过度集中，配合迁移引擎的周期性动态调整，系统可在工作负载变化后的
1--2个控制周期内恢复至均衡状态；（3）\textbf{降级鲁棒性}：在系统全局
高负载时，最短队列降级策略确保每个请求仍能被分配至相对最优的实例，
保证系统在接近满负荷时的服务可用性，而非直接拒绝服务。

算法的时间复杂度为$O(|P|\log|P| + |Q|)$，其中$O(|P|\log|P|)$来自阶段
二的实例排序，$O(|Q|)$来自阶段三的逐请求分配循环（每次分配均为$O(1)$）。
在实际部署中$|P|$通常为数十量级，$|Q|$受调度周期内到达请求数量约束，
算法可在毫秒级内完成一个调度周期的全部决策，满足LLM推理服务的实时调
度需求。
...
\begin{enumerate}
    ...
    \item \textbf{LongBench}\cite{bai2023longbench}（长文本场景）：覆盖21项多样化任务，上下文长度从约2,000 token跨越至超过85,000 token，涵盖多文档问答、文档摘要、少样本学习和代码补全等场景，对KV Cache管理和显存优化策略形成全面压力测试。
\end{enumerate}
...
为补充合成基准测试，本实验使用Azure公开发布的LLM推理生产负载追踪数据\cite{AzureLMM2025}对BanaServe进行评估。该追踪数据包含匿名化的请求级日志，涵盖时间戳、输入/输出token长度和多模态请求类型，工作负载呈现出显著的异构性和突发性，反映了生产环境中真实的LLM服务条件。
...
\begin{figure}[htbp]
    ...
    \caption{Azure生产负载追踪数据前1小时的统计特性}
...
\begin{figure}[htbp]
    ...
    \caption{Azure生产负载追踪实验的吞吐量和延迟综合对比}
...
本章提出了BanaServe，一个面向PD分离架构的动态资源协同优化框架。针对现有PD分离系统中静态资源配置、内生资源不均衡和前缀缓存感知路由引发负载倾斜三类核心局限，BanaServe通过层级权重迁移、注意力级KV Cache迁移和全局KV Cache存储三项关键创新实现了资源分配与状态管理的解耦。

在迁移机制设计上，BanaServe提供两种互补的迁移粒度。层级权重迁移将Transformer层的权重和KV Cache作为整体在GPU间动态迁移，迁移延迟由权重传输主导（$T_{\text{layer}} \approx S^{\text{total}}_\ell / B_{\text{net}} + T_{\text{sync}}$），适合处理严重的工作负载不均衡场景；注意力级KV Cache迁移则沿注意力头维度分割KV Cache，仅传输KV状态而无需迁移模型权重，迁移延迟极低（$T_{\text{attn}} \approx S_{kv}/B_{\text{net}} \ll T_{\text{layer}}$），可在不引发显著服务中断的前提下进行实时细粒度负载调整。两种粒度的迁移机制协同工作，系统根据过载-欠载阈值$\delta$和收益-开销比$\rho$自适应选择最优策略。

在存储架构设计上，BanaServe构建了跨所有Prefill实例的全局KV Cache共享层，通过三阶段层级流水线（预取-计算-存储并行）将通信延迟隐藏于计算过程中（$T_{KV} \ll T_{F,\text{layer}}$）。这一设计从根本上解耦了缓存位置与路由决策的绑定关系，使调度器能够完全基于实时负载进行纯负载感知调度，彻底消除了前缀缓存感知路由引发的热点问题。在此基础上，负载感知请求调度算法基于实时计算和显存综合利用率（$U_d = C_d/C^{\max}_d + M_d/M^{\max}_d$）对Prefill实例进行排序，将请求分发至负载最轻的实例，调度复杂度为$O(|P| \log |P| + |Q|)$，支持实时运行。
