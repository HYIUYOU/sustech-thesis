...
第一章将现有LLM推理系统的核心瓶颈之一归纳为\textbf{挑战①}：批处理策略粗放且部署配置忽视网络拓扑异构性。这一挑战在机器学习即服务（MLaaS）云平台中尤为突出，其根源可进一步分解为部署配置与批处理调度两个相互关联的问题。

在部署配置层面，GPU数量的选择存在双向风险：GPU数量过多时，节点间通信开销随之增大，推理速度反而下降；GPU数量不足时，显存溢出（Out-of-Memory,
\ OOM）直接导致服务中断。即便GPU数量选择合理，层到设备的映射策略（Device Map）若未考虑集群内NVLink与PCIe的带宽差异，仍会造成严重的负载不均，使计算能力强的GPU长时间等待通信瓶颈节点，产生木桶效应。

在批处理调度层面，现有系统普遍采用FIFO策略，既不感知各请求的SLO约束，也不考虑请求输出长度的差异，导致两类浪费同时发生：一方面，输出长度差异大的请求被混编入同一批次，短输出请求须被填充至批内最长输出，产生大量冗余的KV Cache计算和显存占用；另一方面，SLO紧迫的请求与宽松请求混排，既无法优先保障高优先级请求，又因批次整体延迟升高导致大量SLO违约。


\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/uellm_framework.pdf}
  \caption{UELLM系统整体架构图\cite{he2024uellm}}
  \label{fig:uellm_framework}
\end{figure}

针对上述问题，本章提出UELLM（Unified and Efficient LLM Inference Serving）框架，从批处理调度与部署配置两个维度对挑战①进行系统性解决。UELLM由三个协同工作的核心组件构成：\textbf{资源画像器}（Resource Profiler）在请求进入队列前预测其输出长度并提取SLO约束；\textbf{批处理调度器}（BatchScheduler）以输出长度和SLO为双驱动，将请求动态组合为高效批次；\textbf{LLM部署器}（LLM Deployer）基于集群网络拓扑和硬件特性，自动求解最优的层到设备映射策略。三个组件通过在线监控反馈机制形成闭环，实现推理服务的端到端优化，系统整体架构如图~\ref{fig:uellm_framework}所示。
...
\begin{figure}[htbp]
  ...
  \caption{不同GPU数量和批大小配置下归一化延迟、显存占用和GPU利用率的变化}
...
\begin{figure}[htbp]
  ...
  \caption{UELLM与默认批处理算法的对比示意图\cite{he2024uellm}}
...
\begin{algorithm}[htbp]
  \caption{SLO感知与输出长度驱动的动态批处理调度算法(SLO-ODBS)}
  \label{algo:slo_odbs}
...
\begin{figure}[htbp]
  ...
  \caption{UELLM各版本与SOTA在四项核心指标上的综合对比}
